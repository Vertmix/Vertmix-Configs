package com.vertmix.config;

import com.vertmix.config.exception.ConfigNotRegisteredException;
...
public abstract class AbstractConfigRegistry implements ConfigRegistry {
    ...
    protected final Map<Class<?>, Config> loadedConfigs = new ConcurrentHashMap<>();

    protected  <Type> void registerData(Class<Type> clazz, Type type, File file) {
        this.loadedConfigs.put(clazz, new Config(file, type));
    }
    ...
    public void save(Object obj) {
        if (this.loadedConfigs.containsKey(obj.getClass())) {
            save(obj, this.loadedConfigs.get(obj.getClass()).file());
    ...
    public <Type> Optional<Type> load(Class<Type> clazz) {
        if (this.loadedConfigs.containsKey(clazz)) {
            return load(clazz, this.loadedConfigs.get(clazz).file());
        } else throw new ConfigNotRegisteredException();
    }

    @Override
    public Collection<Object> getConfigs() {
        return this.loadedConfigs.values().stream().map(Config::obj).collect(Collectors.toList());
    }
